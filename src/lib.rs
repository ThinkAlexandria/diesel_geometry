...
//! You cannot use `infer_schema!` with `diesel_geometry` types at this time.
//!
//! If you use diesel print-schema, you must manually modify the generated code to export the
//! `diesel_geometry` sql types inside each `table!` macro that uses diesel_geometry types. Because exporting any types inside the
//! table macro overrides the default exports, you must also manually export the diesel sql types
//! as well.
